...
async function main() {
    ...
    if (!site) {
        ...
        site = await prisma.site.create({
            data: {
                ...
                allowedOrigins: ['proyectodomus.pe', 'www.proyectodomus.pe', 'localhost:5173'],
            },
        });
    }
    const existente = await prisma.form.findFirst({ where: { organizationId: org.id } });
    const form = existente ??
        (await prisma.form.create({
            data: {
                ...
                projectId: domus.id,
                name: 'Contacto Domus',
                schema: FORMULARIO_DOMUS,
                notifyEmails: ['ventas@bastion.pe'],
            },
        }));
    console.log('\n  Semilla lista.\n');
    ...
    if (secretKey)
        console.log('  Secret key   :', secretKey, ' <- se muestra una sola vez');
