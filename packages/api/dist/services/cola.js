...
async function procesar(job) {
    switch (job.type) {
        case 'email.send':
            return enviarEmail(job.payload);
        case 'sla.check':
            return revisarSla(job.payload);
        case 'meta.lead.fetch': {
            ...
            return procesarLeadgen(String(job.payload.leadgenId));
        }
...
async function enviarEmail(payload) {
    ...
    if (!lead)
        return;
    const destinatarios = new Set(Array.isArray(payload.to) ? payload.to : []);
    if (lead.owner?.email)
        destinatarios.add(lead.owner.email);
...
async function revisarSla(payload) {
    ...
    if (!lead || lead.firstContactAt || lead.status !== 'activo')
        return;
