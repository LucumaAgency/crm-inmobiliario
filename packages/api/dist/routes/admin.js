...
export default async function adminRoutes(app) {
        ...
        const actualizada = await prisma.metaPage.update({
            ...
            data: {
                ...
                ...(projectId !== undefined
                    ? { projectId: await proyectoValido(req.user.organizationId, projectId) }
                    : {}),
                ...(formMap !== undefined ? { formMap: formMap } : {}),
                ...(notifyEmails !== undefined ? { notifyEmails: notifyEmails } : {}),
                // Un token nuevo borra el último error: es lo que se venía a arreglar.
                ...(accessToken
                    ? {
                        accessTokenEnc: cifrar(await (await import('../services/meta.js')).resolverTokenDePagina(pagina.pageId, accessToken)),
                        lastError: null,
                    }
                    : {}),
            },
    ...
    app.post('/meta/leads/:id/retry', { preHandler: gestion }, async (req, reply) => {
        ...
        if (!aviso)
            return reply.code(404).send({ error: 'Aviso no encontrado' });
        if (aviso.status === 'procesado')
            return reply.code(409).send({ error: 'Ese aviso ya entró como lead.' });
    ...
    app.post('/whatsapp/numbers', { preHandler: gestion }, async (req, reply) => {
        ...
        if (ocupado)
            return reply.code(409).send({ error: 'Ese número ya está conectado.' });
    ...
    app.patch('/whatsapp/numbers/:id', { preHandler: gestion }, async (req, reply) => {
        ...
        if (!numero)
            return reply.code(404).send({ error: 'Número no encontrado' });
        const { accessToken, projectId, ...resto } = parsed.data;
    ...
    app.get('/whatsapp/templates', async (req) => {
        ...
        if (!numero)
            return { ok: false, error: 'No hay ningún número conectado' };
    ...
    async function proyectoValido(organizationId, projectId) {
        if (!projectId)
            return null;
