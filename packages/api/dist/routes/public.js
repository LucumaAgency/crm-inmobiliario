import { formSchema, submissionInput } from '@lucuma-crm/shared';
...
export default async function publicRoutes(app) {
    async function sitePorPublicKey(req) {
        ...
        if (candidato && !originAllowed(candidato, site.allowedOrigins))
            return null;
        return site;
    }
    async function sitePorSecret(req) {
    ...
    app.get('/forms/:id', async (req, reply) => {
        ...
        if (!parsed.success)
            return reply.code(500).send({ error: 'Esquema del formulario inválido' });
        const out = {
            id: form.id,
    ...
    app.get('/forms', async (req, reply) => {
        ...
        if (!site)
            return reply.code(401).send({ error: 'Secret key inválida' });
    ...
    app.post('/forms/:id/submissions', async (req, reply) => {
