...
async function main() {
  ...
  await prisma.user.update({
    ...
    data: {
      ...
      ...(comoAdmin ? { role: 'admin_lucuma' as const, active: true } : {}),
    },
  });
  if (comoAdmin) Object.assign(user, { role: 'admin_lucuma', active: true });
...
async function crearAdmin(email: string, slug?: string) {
