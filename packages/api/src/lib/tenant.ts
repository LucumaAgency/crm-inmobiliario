...
const TTL_MS = 60_000;
const cache = new Map<string, { valor: Tenant | null; expira: number }>();

/** Deja el host sin puerto y en minúsculas. */
function limpiarHost(host: string | undefined): string {
 ...
 */
export function slugDesdeHost(host: string | undefined, base: string): string | null {
  ...
  if (etiqueta === 'www') return null;
  return etiqueta;
}

/** Busca la organización del subdominio, con caché corta. */
export async function resolverTenant(host: string | undefined): Promise<Tenant | null> {
...
export function olvidarTenant(slug?: string) {
 ...
 */
export function sesionCoincide(req: FastifyRequest, organizationId: string): boolean {
...
export function urlDeOrganizacion(slug: string | null | undefined): string {
  if (!env.baseDomain || !slug) return env.appUrl.replace(/\/$/, '');
