 ...
 */
const PERMITIDOS: Record<string, { ext: string; firma: (b: Buffer) => boolean }> = {
...
): Promise<Guardado | { error: string }> {
  if (contenido.length === 0) return { error: 'El archivo está vacío.' };
...
export function urlPublica(ruta: string | null | undefined): string | null {
  if (!ruta) return null;
  if (/^https?:\/\//i.test(ruta)) return ruta; // URLs externas de antes de esto
 ...
 */
export async function borrarSiHuerfano(ruta: string | null | undefined, sigueEnUso: boolean) {
  if (!ruta || sigueEnUso || /^https?:\/\//i.test(ruta)) return;
  try {
    await fs.promises.unlink(path.join(uploadsDir, ruta));
  } catch (err) {
    const e = err as NodeJS.ErrnoException;
    if (e.code !== 'ENOENT') logError('[media] no se pudo borrar', ruta, e.message);
  }
}
