 ...
 */
export async function recibirMensajes(valor: ValorMensajes): Promise<void> {
   ...
   */
  logLine(
    `wa: aviso recibido de ${phoneNumberId} ` +
...
async function recibirUno(
  numero: { id: string; organizationId: string; projectId: string | null; phoneNumberId: string },
  valor: ValorMensajes,
  mensaje: MensajeEntrante
) {
  const waId = mensaje.from;
  if (!waId || !mensaje.id) return;
  ...
  await prisma.$transaction([
    prisma.waMessage.create({
      data: {
        ...
        type: tipo,
        body: texto,
        media: (media ?? undefined) as never,
        status: 'entregado',
        raw: mensaje as never,
        sentAt: recibidoEn,
      },
    }),
...
async function aplicarEstado(estado: EstadoMensaje) {
  if (!estado.id || !estado.status) return;
  ...
  if (nuevo !== 'fallido' && orden.indexOf(nuevo) <= orden.indexOf(mensaje.status)) return;
...
export function ventanaAbierta(conv: { windowExpiresAt: Date | null }): boolean {
 ...
 */
export async function enviarTexto(datos: EnvioTexto) {
  const conv = await cargarConversacion(datos.conversationId);
  if (!ventanaAbierta(conv)) {
    throw Object.assign(
      new Error(
        'La ventana de 24 horas se cerró: solo se puede escribir con una plantilla aprobada.'
      ),
      { statusCode: 409 }
    );
  }
...
export async function enviarPlantilla(datos: EnvioPlantilla) {
  const conv = await cargarConversacion(datos.conversationId);
  return crearYEncolar(conv, {
    userId: datos.userId,
    type: 'template',
...
async function crearYEncolar(
  ...
  const mensaje = await prisma.waMessage.create({
    data: {
      ...
      userId: datos.userId,
      status: 'pendiente',
      raw: (datos.payload ?? undefined) as never,
    },
  });
 ...
 */
export async function despacharMensaje(messageId: string): Promise<void> {
  const mensaje = await prisma.waMessage.findUnique({
    where: { id: messageId },
    include: { conversation: { include: { waNumber: true } } },
  });
  if (!mensaje) throw new Error(`wa: no existe el mensaje ${messageId}`);
  if (mensaje.status !== 'pendiente') return; // ya salió: la cola reintentó de más

  const numero = mensaje.conversation.waNumber;
  const cuerpo =
    mensaje.type === 'template'
      ? cuerpoPlantilla(mensaje.conversation.waId, mensaje.raw as Record<string, unknown>)
      : {
  ...
  try {
    const res = await llamarCloud(
      `${numero.phoneNumberId}/messages`,
      descifrar(numero.accessTokenEnc),
      cuerpo
    );
    const waMessageId = (res as { messages?: Array<{ id?: string }> }).messages?.[0]?.id;
...
export async function plantillasDe(phoneNumberId: string) {
  const numero = await prisma.waNumber.findUnique({ where: { phoneNumberId } });
  ...
  try {
    ...
    return { ok: true as const, templates: res.data ?? [] };
  } catch (err) {
...
async function cargarConversacion(id: string) {
  ...
  if (!conv) throw Object.assign(new Error('Conversación no encontrada'), { statusCode: 404 });
  return conv;
}
...
async function conversacionDe(
  numero: { id: string; organizationId: string },
  waId: string,
  nombre?: string
) {
...
function interpretar(m: MensajeEntrante): {
...
function cuerpoPlantilla(waId: string, payload: Record<string, unknown> | null) {
  const nombre = String(payload?.name ?? '');
  const idioma = String(payload?.language ?? 'es');
  const variables = Array.isArray(payload?.variables) ? (payload!.variables as string[]) : [];
  return {
    messaging_product: 'whatsapp',
    to: waId,
    type: 'template',
...
function resumenPlantilla(nombre: string, variables?: string[]) {
  const v = (variables ?? []).filter(Boolean);
  return v.length ? `[plantilla ${nombre}] ${v.join(' · ')}` : `[plantilla ${nombre}]`;
}

async function llamarCloud(ruta: string, token: string, cuerpo?: unknown): Promise<unknown> {
  ...
  try {
    ...
    if (!res.ok || datos.error) {
      ...
      throw Object.assign(
        new Error(
          `Cloud API ${res.status}: ${e?.message ?? 'sin detalle'}${
