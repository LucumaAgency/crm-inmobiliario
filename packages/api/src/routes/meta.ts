...
import { recibirMensajes, type ValorMensajes } from '../services/whatsapp.js';

/** Cuerpo del aviso, según la documentación de Meta. */
...
export default async function metaRoutes(app: FastifyInstance) {
  ...
  app.post('/webhook', async (req: FastifyRequest<{ Body: CuerpoWebhook }>, reply) => {
    ...
    if (cuerpo.object !== 'page') return { received: true };
...
function firmaValida(req: FastifyRequest): boolean {
 ...
 */
function seguroIguales(a: string | undefined, b: string): boolean {
