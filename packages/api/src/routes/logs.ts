  ...
  app.get<{ Querystring: { lines?: string; q?: string } }>(
    ...
    async (req) => {
      ...
      try {
        ...
        if (desde > 0) contenido = contenido.slice(contenido.indexOf('\n') + 1);
      } catch {
      ...
      if (filtro) lineas = lineas.filter((l) => l.toLowerCase().includes(filtro));
 ...
 */
function redactar(linea: string): string {
  return linea
