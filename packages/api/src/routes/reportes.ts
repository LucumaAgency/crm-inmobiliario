...
function indicadores(leads: FilaLead[], ganadas: Set<string>) {
  const contactados = leads.filter((l) => l.firstContactAt);
  const minutos = contactados.map(
    (l) => (l.firstContactAt!.getTime() - l.createdAt.getTime()) / 60000
  );
  return {
    leads: leads.length,
...
export default async function reportesRoutes(app: FastifyInstance) {
  ...
  app.get<{ Querystring: { dias?: string; proyecto?: string } }>('/resumen', async (req) => {
    ...
    const porDia = new Map<string, { leads: number; contactados: number; ganados: number }>();
    for (const f of filas) {
      const d = diaLima(f.createdAt);
      const v = porDia.get(d) ?? { leads: 0, contactados: 0, ganados: 0 };
      v.leads++;
      if (f.firstContactAt) v.contactados++;
      if (f.stageId && ganadas.has(f.stageId)) v.ganados++;
      porDia.set(d, v);
    }
    ...
    const contar = <K extends keyof FilaLead>(lista: FilaLead[], clave: K) => {
      const m = new Map<string, number>();
      for (const f of lista) {
        const k = (f[clave] as string | null) ?? '';
        m.set(k, (m.get(k) ?? 0) + 1);
      }
      return m;
    };
