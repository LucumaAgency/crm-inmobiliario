...
type UsuarioBase = Pick<SessionUser, 'id' | 'organizationId' | 'role' | 'email' | 'name'>;

async function abrirSesion(
  req: FastifyRequest,
  reply: FastifyReply,
  user: UsuarioBase,
  metodo: 'enlace' | 'password'
) {
...
export default async function authRoutes(app: FastifyInstance) {
  ...
  app.post<{ Body: { token?: string } }>('/callback', async (req, reply) => {
  ...
  app.post('/login', async (req, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: 'Correo o contraseña inválidos' });
  ...
  app.post('/password', { preHandler: requireAuth }, async (req, reply) => {
    ...
    if (user.passwordHash) {
      ...
      if (!ok) return reply.code(403).send({ error: 'La contraseña actual no es correcta' });
    }
